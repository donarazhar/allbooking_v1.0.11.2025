...
@section('content')
    <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-blue-100">
                        @if ($isSuperAdmin)
                            Data semua pengguna dan booking dari semua cabang
                        @else
                            Data pengguna dan booking di cabang Anda
                        @endif
                    </p>
                </div>
        ...
        {{-- Filter --}}
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <form method="GET" action="{{ route('admin.laporan.pengguna') }}"
                class="grid grid-cols-1 md:grid-cols-{{ $isSuperAdmin ? '5' : '4' }} gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($users as $index => $user)
                            @php
                                // Filter bookings by cabang if Admin Cabang
                                if (!$isSuperAdmin) {
                                    $userBookings = $user->transaksiBooking->where(
                                        'cabang_id',
                                        Auth::user()->cabang_id,
                                    );
                                } else {
                                    $userBookings = $user->transaksiBooking;
                                }
            ...
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
                <div class="text-sm text-gray-600">
                    Total: <span class="font-medium">{{ $users->count() }}</span> pengguna
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const userCabangId = {{ !$isSuperAdmin ? Auth::user()->cabang_id : 'null' }};
        ...
        function viewDetail(user) {
            ...
            // Filter bookings by cabang if Admin Cabang
            let bookings = user.transaksi_booking || [];
            if (!isSuperAdmin && userCabangId) {
                bookings = bookings.filter(b => b.cabang_id === userCabangId);
            }
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-700">
                                    <i class="fas fa-building mr-1"></i>${booking.cabang?.nama || '-'}
                                </span>
                            </div>
