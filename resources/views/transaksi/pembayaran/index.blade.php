...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Filter Jenis Bayar</label>
            ...
            <table class="w-full" id="pembayaranTable">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Booking Detail</th>
                ...
                <tbody class="divide-y divide-gray-200" id="tableBody">
                    @forelse($pembayaran as $index => $item)
                        <tr class="hover:bg-gray-50 transition-colors pembayaran-row" 
                            data-user="{{ strtolower($item->bookings->user->nama ?? '') }}"
                            data-jenis="{{ $item->jenis_bayar }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-8 w-8 bg-gradient-to-br from-purple-400 to-purple-600 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                        {{ strtoupper(substr($item->bookings->user->nama ?? 'U', 0, 2)) }}
                                    </div>
                                    <div class="ml-3">
                                        <p class="text-sm font-medium text-gray-900">{{ $item->bookings->user->nama ?? '-' }}</p>
                                        <p class="text-xs text-gray-500">{{ $item->bookings->user->email ?? '-' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm">
                                @if($item->bookings && $item->bookings->bukaJadwal)
                                   
                                    <p class="font-medium text-gray-900">
                                        {{ $item->bookings->bukaJadwal->hari ?? '-' }}, 
                                        {{ $item->bookings->bukaJadwal->tanggal ? \Carbon\Carbon::parse($item->bookings->bukaJadwal->tanggal)->format('d M Y') : '-' }}
                                        {{ $item->bookings->bukaJadwal->sesi->nama ?? '-' }} | 
                                    </p>
                                    <p class="text-xs text-gray-500">{{ $item->bookings->bukaJadwal->jenisAcara->nama ?? '-' }}</p>
                                @else
                            ...
                            <td class="px-6 py-4 text-sm text-right">
                                <span class="inline-flex items-center px-3 py-1 bg-green-50 text-green-700 rounded-lg font-bold">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form action="{{ route('admin.transaksi.pembayaran.destroy', $item->id) }}" method="POST" 
                                          class="inline" 
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
            <div class="flex justify-between items-center">
                ...
                <div class="text-sm font-medium text-gray-900">
                    Total: <span class="text-green-600">Rp {{ number_format($pembayaran->sum('nominal'), 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
    </div>
...
<div id="addModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form id="pembayaranForm" method="POST" action="{{ route('admin.transaksi.pembayaran.store') }}" enctype="multipart/form-data" class="p-6 space-y-4">
            @csrf
            ...
            <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
                <div class="flex">
                    ...
                    <div class="text-sm text-blue-700">
                        ...
                        <p class="mt-1">Pilih booking yang aktif untuk melakukan pembayaran. Upload bukti transfer untuk verifikasi.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="booking_id" name="booking_id" required
                        ...
                        @foreach(\App\Models\Booking::with(['user', 'bukaJadwal.sesi', 'bukaJadwal.jenisAcara'])->where('status_booking', 'active')->orderBy('tgl_booking', 'desc')->get() as $b)
                            <option value="{{ $b->id }}">
                                ...
                                @else
                                    Booking #{{ $b->id }} | {{ \Carbon\Carbon::parse($b->tgl_booking)->format('d M Y') }}
                                @endif
                            </option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-500 mt-1">
                        ...
                        Hanya menampilkan booking dengan status active
                    </p>
                </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" id="nominal" name="nominal" required min="1000" max="999999999" step="1000"
                           placeholder="0"
            ...
            <div>
                ...
                <input type="file" id="bukti_bayar" name="bukti_bayar" accept="image/jpeg,image/png,image/jpg"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="border-t border-gray-200 pt-4">
                ...
                <div id="detail_bukti_container" class="bg-gray-50 rounded-lg p-4">
                    <img id="detail_bukti" src="" alt="Bukti Pembayaran" class="max-w-full rounded-lg border hidden cursor-pointer" onclick="viewBukti(this.src)">
...
<div id="buktiModal" class="hidden fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center p-4">
    <div class="relative max-w-4xl w-full">
        <button onclick="closeModal('buktiModal')" class="absolute -top-12 right-0 text-white hover:text-gray-300 transition-colors">
...
function confirmDelete(jenis, nominal) {
    return confirm(`Apakah Anda yakin ingin menghapus pembayaran:\n\nJenis: ${jenis}\nNominal: Rp ${nominal}\n\nData yang sudah dihapus tidak dapat dikembalikan.`);
}
...
function viewDetail(data) {
    ...
    document.getElementById('detail_tanggal').textContent = new Date(data.tgl_pembayaran).toLocaleDateString('id-ID', { 
    ...
    if (data.booking && data.booking.buka_jadwal) {
        document.getElementById('detail_acara').textContent = data.booking.buka_jadwal.jenis_acara ? 
            data.booking.buka_jadwal.jenis_acara.nama : '-';
        document.getElementById('detail_jadwal').textContent = data.booking.buka_jadwal.hari + ', ' + 
            new Date(data.booking.buka_jadwal.tanggal).toLocaleDateString('id-ID');
        document.getElementById('detail_sesi').textContent = data.booking.buka_jadwal.sesi ? 
            data.booking.buka_jadwal.sesi.nama : '-';
    }
