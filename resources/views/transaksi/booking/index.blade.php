...
@section('content')
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Tanggal Booking
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">User</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Jadwal</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Catering</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Batas Bayar</th>
                        ...
                        @forelse($bookings as $index => $item)
                            <tr class="hover:bg-gray-50 transition-colors"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div
                                            class="flex-shrink-0 h-8 w-8 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                    ...
                                    @if ($item->bukaJadwal)
                                        <div class="space-y-1">
                                            ...
                                            <p class="text-xs text-gray-500">
                                                <i class="fas fa-clock mr-1"></i>{{ $item->bukaJadwal->sesi->nama ?? '-' }}
                                            </p>
                                            <p class="text-xs font-medium text-blue-600">
                                                <i
                                                    class="fas fa-calendar-check mr-1"></i>{{ $item->bukaJadwal->jenisAcara->nama ?? '-' }}
                                            </p>
                                            <span
                                                ...
                                                <i
                                                    class="fas {{ $item->bukaJadwal->status_jadwal === 'booked' ? 'fa-lock' : 'fa-unlock' }} mr-1 text-xs"></i>
                                ...
                                <td class="px-6 py-4 text-sm">
                                    @if ($item->tgl_expired_booking)
                                        ...
                                        <div class="flex flex-col">
                                            <span
                                                class="font-medium text-gray-900">{{ $expiredDate->format('d M Y') }}</span>
                                            @if ($daysLeft < 0)
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if ($item->status_booking === 'inactive')
                                        <span
            ...
            <form id="bookingForm" method="POST" action="{{ route('admin.transaksi.booking.store') }}"
                    ...
                    <div class="flex">
                        ...
                        <div class="text-sm text-blue-700">
                            ...
                            <ul class="mt-1 space-y-1 list-disc list-inside">
                                <li>Status jadwal akan <strong>otomatis berubah menjadi "booked"</strong> setelah booking
                                    dibuat</li>
                                <li>Batas pembayaran otomatis <strong>2 minggu (14 hari)</strong> dari tanggal booking</li>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="bukajadwal_id" name="bukajadwal_id" required
                            ...
                            @foreach (\App\Models\BukaJadwal::with(['sesi', 'jenisAcara'])->where('status_jadwal', 'available')->orderBy('tanggal')->get() as $bj)
                                <option value="{{ $bj->id }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" id="tgl_booking" name="tgl_booking" required value="{{ date('Y-m-d') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
    ...
    <script>
        ...
        function editData(data) {
            ...
            document.getElementById('bookingForm').action = '/admin/transaksi/booking/' + data.id;
        ...
        //     document.getElementById('statusForm').action = '/admin/transaksi/booking/' + data.id + '/update-status';
