...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-green-500 to-green-700 rounded-xl shadow-lg p-8 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-green-100">Data pembayaran dan transaksi keuangan di
                        {{ $cabangList->first()->nama ?? 'cabang Anda' }}</p>
                    @if ($cabangList->first())
                        <p class="text-green-200 text-sm mt-1">
                            <i class="fas fa-map-marker-alt mr-1"></i>{{ $cabangList->first()->alamat ?? '-' }}
                        </p>
                    @endif
                </div>
        ...
        {{-- Filter --}}
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <form method="GET" action="{{ route('pimpinan.laporan.keuangan') }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-gray-400 text-sm mt-1">Belum ada transaksi pembayaran di cabang ini</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-600">
                        Total: <span class="font-medium">{{ $pembayarans->count() }}</span> transaksi pembayaran di
                        {{ $cabangList->first()->nama ?? 'cabang ini' }}
                    </div>
        ...
        <div class="bg-green-50 border-l-4 border-green-500 rounded-lg p-6">
            <div class="flex items-start">
                ...
                <div>
                    <h4 class="font-semibold text-green-900 mb-2">Informasi Laporan</h4>
                    <ul class="text-sm text-green-800 space-y-1">
                        <li>• Laporan ini menampilkan data pembayaran di
                            <strong>{{ $cabangList->first()->nama ?? 'cabang Anda' }}</strong></li>
                        <li>• Total pendapatan dihitung hanya untuk cabang ini</li>
                        <li>• Gunakan filter untuk melihat data dalam periode atau jenis pembayaran tertentu</li>
