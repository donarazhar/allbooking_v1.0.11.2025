...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-700 rounded-xl shadow-lg p-8 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-blue-100">Data pengguna dan booking di {{ $cabangInfo->nama ?? 'cabang Anda' }}</p>
                    @if ($cabangInfo)
                        <p class="text-blue-200 text-sm mt-1">
                            <i class="fas fa-map-marker-alt mr-1"></i>{{ $cabangInfo->alamat ?? '-' }}
                        </p>
                    @endif
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Pernah booking di cabang ini</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Memiliki booking</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Di cabang ini</p>
                    </div>
        ...
        {{-- Filter --}}
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <form method="GET" action="{{ route('pimpinan.laporan.pengguna') }}"
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-center text-xs font-semibold text-gray-700 uppercase">Total Booking
                            </th>
                            <th class="px-6 py-4 text-right text-xs font-semibold text-gray-700 uppercase">Total Bayar</th>
                            <th class="px-6 py-4 text-center text-xs font-semibold text-gray-700 uppercase no-print">Aksi
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @forelse($users as $index => $user)
                            @php
                                // Filter bookings by cabang pimpinan
                                $userBookings = $user->transaksiBooking->where('cabang_id', $cabangInfo->id);

                                // Calculate total bayar from filtered bookings
                                $totalBayar = $userBookings->sum(function ($booking) {
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div
                                            class="h-10 w-10 bg-blue-100 rounded-full flex items-center justify-center mr-3 flex-shrink-0">
                                            <span
                                                class="text-blue-600 font-bold text-sm">{{ strtoupper(substr($user->nama, 0, 2)) }}</span>
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4 text-center no-print">
                                    <button onclick='viewDetail(@json($user), {{ $cabangInfo->id }})'
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-gray-400 text-sm mt-1">Belum ada user yang booking di cabang ini</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-600">
                        Total: <span class="font-medium">{{ $users->count() }}</span> pengguna di
                        {{ $cabangInfo->nama ?? 'cabang ini' }}
                    </div>
                    @if (request('start_date') || request('end_date'))
                        <div class="text-xs text-gray-500">
                            <i class="fas fa-filter mr-1"></i>Terfilter
        ...
        <div class="bg-purple-50 border-l-4 border-purple-500 rounded-lg p-6">
            <div class="flex items-start">
                ...
                <div>
                    <h4 class="font-semibold text-purple-900 mb-2">Informasi Laporan</h4>
                    <ul class="text-sm text-purple-800 space-y-1">
                        <li>• Laporan ini menampilkan data pengguna yang pernah melakukan booking di
                            <strong>{{ $cabangInfo->nama ?? 'cabang Anda' }}</strong></li>
                        <li>• Total booking dan pembayaran dihitung hanya untuk cabang ini</li>
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    <h4 class="font-semibold text-gray-900 mb-3">
                        ...
                        Riwayat Booking di {{ $cabangInfo->nama ?? 'Cabang Ini' }}
                    </h4>
    ...
    <script>
        function viewDetail(user, cabangId) {
            ...
            // Filter bookings untuk cabang pimpinan saja
            const filteredBookings = user.transaksi_booking.filter(b => b.cabang_id === cabangId);

            if (filteredBookings.length > 0) {
                let html = '';
                filteredBookings.forEach((booking, index) => {
                    const totalBayar = booking.transaksi_pembayaran.reduce((sum, p) => sum + parseFloat(p.nominal),
                        0);
                    const statusClass = booking.status_booking === 'active' ? 'bg-green-100 text-green-700' :
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div class="flex items-start justify-between mb-2">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 mt-1">
                                ${booking.buka_jadwal?.hari || '-'}, ${booking.buka_jadwal?.tanggal ? new Date(booking.buka_jadwal.tanggal).toLocaleDateString('id-ID') : '-'}
                            </p>
                            <p class="text-xs text-gray-500">
                                <i class="fas fa-clock mr-1"></i>${booking.buka_jadwal?.sesi?.nama || '-'}
                            </p>
                        </div>
                    ...
                    <div class="flex items-center justify-between text-sm mt-3 pt-3 border-t border-gray-200">
                        <span class="text-gray-600">Total Bayar:</span>
                        <span class="font-bold text-green-600">Rp ${new Intl.NumberFormat('id-ID').format(totalBayar)}</span>
                    </div>
                    <div class="mt-2 text-xs text-gray-500">
                        ${booking.transaksi_pembayaran.length} pembayaran
                    </div>
                </div>
            `;
                });
                ...
                bookingsList.innerHTML = '<p class="text-center text-gray-500 py-4">Belum ada booking di cabang ini</p>';
            }
