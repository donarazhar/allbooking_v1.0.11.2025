        ...
        <div class="space-y-6">
            @forelse($bookings as $booking)
                @php
                    ...
                    // Calculate days left for expired date
                    $daysLeft = 0;
                                ...
                                <div class="flex items-center gap-3 mb-2 flex-wrap">
                                    ...
                                    @elseif($sudahBayarDP)
                                        <span
                                            ...
                                            <i class="fas fa-coins mr-1"></i>DP Dibayar
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700">
                                            <i class="fas fa-exclamation-triangle mr-1"></i>Belum DP
                                        </span>
                                    @endif
                                </div>
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                @else
                                    <div class="bg-gray-50 rounded-lg p-8 text-center border border-gray-200">
                                        ...
                                        <p class="text-gray-400 text-sm mt-1">Lakukan pembayaran DP untuk mengaktifkan
                                            booking</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="bg-blue-50 border-l-4 border-blue-500 rounded-lg p-4">
                                    ...
                                    @if (!$sudahBayarDP)
                                        <div class="flex items-start">
                                            <i
                                                class="fas fa-exclamation-circle text-yellow-500 mr-3 mt-0.5 flex-shrink-0"></i>
                                            <div>
                                                <p class="text-sm text-blue-800 font-medium">Belum Bayar DP</p>
                                                <p class="text-xs text-blue-700 mt-1">Segera bayar DP untuk mengaktifkan
                                                    booking Anda</p>
                                            </div>
                                        </div>
                                    @elseif(!$sudahPelunasan)
                                        <div class="flex items-start">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-blue-700 mt-1">Lanjutkan dengan termin atau pelunasan
                                                </p>
                                            </div>
                                        </div>
                                    @else
                                        <div class="flex items-start">
                                            <i class="fas fa-check-double text-green-500 mr-3 mt-0.5 flex-shrink-0"></i>
                                            <div>
                                                <p class="text-sm text-blue-800 font-medium">Lunas</p>
                                                <p class="text-xs text-blue-700 mt-1">Pembayaran sudah selesai. Terima
                                                    kasih!</p>
                                            </div>
                                        </div>
                                    @endif
                                ...
                                @if ($booking->tgl_expired_booking && !$sudahBayarDP)
                                    <div
                                        class="{{ $daysLeft < 0 ? 'bg-red-50 border-red-500' : ($daysLeft <= 3 ? 'bg-orange-50 border-orange-500' : 'bg-yellow-50 border-yellow-500') }} border-l-4 rounded-lg p-4">
                                        <div class="flex items-start">
                                            <i
                                                class="fas fa-clock {{ $daysLeft < 0 ? 'text-red-500' : ($daysLeft <= 3 ? 'text-orange-500' : 'text-yellow-500') }} mr-3 mt-0.5 flex-shrink-0"></i>
                                            <div>
                                                @if ($daysLeft < 0)
                                                    <p class="text-sm font-semibold text-red-700">Booking Expired!</p>
                                                    <p class="text-xs text-red-600 mt-1">Batas pembayaran sudah lewat
                                                        {{ abs($daysLeft) }} hari yang lalu</p>
                                                @elseif($daysLeft <= 3)
                                                    <p class="text-sm font-semibold text-orange-700">Segera Bayar DP!</p>
                                                    <p class="text-xs text-orange-600 mt-1">⚠️ Sisa {{ $daysLeft }}
                                                        hari lagi</p>
                                                @else
                                                    <p class="text-sm font-semibold text-yellow-700">Jangan Lupa Bayar DP
                                                    </p>
                                                    <p class="text-xs text-yellow-600 mt-1">Sisa {{ $daysLeft }} hari
                                                        lagi</p>
                                                @endif
                                                <p
                                                    class="text-xs {{ $daysLeft < 0 ? 'text-red-600' : ($daysLeft <= 3 ? 'text-orange-600' : 'text-yellow-600') }} mt-1">
                                                    Batas:
                                ...
                                @if (!$sudahPelunasan)
                                    <button onclick='openBayarModal(@json($booking))'
                                        class="w-full px-6 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-all shadow-sm hover:shadow-md font-medium text-lg">
                                ...
                                @else
                                    <div class="bg-green-50 border-2 border-green-200 rounded-lg p-4 text-center">
                                        ...
                                        <p class="text-green-700 font-semibold">Pembayaran Selesai</p>
                                        <p class="text-green-600 text-sm mt-1">Booking Anda sudah lunas</p>
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            @empty
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                    ...
                    <p class="text-gray-500 mb-6">Anda belum memiliki booking aktif yang perlu dibayar</p>
                    <a href="{{ route('user.booking') }}"
                        class="inline-block px-6 py-3 bg-primary text-white rounded-lg hover:bg-blue-700 transition-colors font-medium shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <ul class="text-sm text-blue-800 space-y-2">
                    <li class="flex items-start">
                        ...
                        <span><strong>DP (Down Payment)</strong> wajib dibayar agar status booking menjadi
                    ...
                    <li class="flex items-start">
                        <i class="fas fa-check text-blue-600 mr-2 mt-0.5 flex-shrink-0"></i>
                        <span>Upload bukti pembayaran (foto transfer) yang <strong>jelas dan terbaca</strong></span>
                    </li>
                </ul>
                <ul class="text-sm text-blue-800 space-y-2">
                    <li class="flex items-start">
                        <i class="fas fa-check text-blue-600 mr-2 mt-0.5 flex-shrink-0"></i>
                        <span>Admin akan memverifikasi pembayaran Anda dalam <strong>1x24 jam</strong></span>
                    </li>
                    <li class="flex items-start">
                        <i class="fas fa-check text-blue-600 mr-2 mt-0.5 flex-shrink-0"></i>
                        <span>Anda bisa membayar secara bertahap: <strong>DP → Termin 1-4 → Pelunasan</strong></span>
                    </li>
                    <li class="flex items-start">
                        <i class="fas fa-check text-blue-600 mr-2 mt-0.5 flex-shrink-0"></i>
                        <span>Booking expired dalam <strong>2 minggu</strong> jika tidak ada pembayaran DP</span>
                    </li>
                </ul>
            </div>
        </div>
            ...
            <form action="{{ route('user.bayar.store') }}" method="POST" enctype="multipart/form-data" id="bayarForm"
                ...
                <div class="bg-gradient-to-r from-green-50 to-green-100 rounded-lg p-4 border border-green-200">
                    ...
                    <div class="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div>
                            <p class="text-green-700 font-medium">Sesi:</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="tgl_pembayaran" id="tgl_pembayaran" required
                            value="{{ date('Y-m-d') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                        <p class="text-xs text-gray-500 mt-1">
                            <i class="fas fa-info-circle mr-1"></i>Tanggal Anda melakukan transfer
                        </p>
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="nominal" id="nominal" required min="1000" max="999999999"
                            step="1000" placeholder="0" oninput="formatNominalPreview(this.value)"
                            class="w-full pl-12 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    </div>
                    <p class="text-xs text-gray-500 mt-1">
                        <i class="fas fa-info-circle mr-1"></i>Min: Rp 1.000 | Max: Rp 999.999.999
                    </p>
                    <div id="nominalPreview" class="hidden mt-2 text-sm text-green-600 font-medium"></div>
                </div>
                ...
                <div>
                    ...
                    <input type="file" name="bukti_bayar" id="bukti_bayar" accept="image/jpeg,image/png,image/jpg"
                        ...
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    <p class="text-xs text-gray-500 mt-1">
                        <i class="fas fa-image mr-1"></i>Upload foto bukti transfer (Max: 2MB, Format: JPG, PNG)
                    </p>
                    <div id="imagePreview" class="mt-2 hidden">
                        <img id="previewImg" src="" alt="Preview" class="max-w-xs rounded-lg border shadow-sm">
                    </div>
                </div>

                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                    <div class="flex">
                        <i class="fas fa-exclamation-triangle text-yellow-600 mr-3 mt-0.5 flex-shrink-0"></i>
                        <div class="text-sm text-yellow-800">
                            <p class="font-medium mb-2">Penting!</p>
                            <ul class="space-y-1">
                                <li class="flex items-start">
                                    <i class="fas fa-check text-yellow-600 mr-2 mt-0.5 text-xs"></i>
                                    <span>Pastikan bukti pembayaran <strong>jelas dan dapat dibaca</strong></span>
                                </li>
                                <li class="flex items-start">
                                    <i class="fas fa-check text-yellow-600 mr-2 mt-0.5 text-xs"></i>
                                    <span>Setelah bayar <strong>DP</strong>, status booking akan menjadi
                                        <strong>Active</strong></span>
                                </li>
                                <li class="flex items-start">
                                    <i class="fas fa-check text-yellow-600 mr-2 mt-0.5 text-xs"></i>
                                    <span>Admin akan memverifikasi dalam <strong>1x24 jam</strong></span>
                                </li>
                                <li class="flex items-start">
                                    <i class="fas fa-check text-yellow-600 mr-2 mt-0.5 text-xs"></i>
                                    <span>Nominal yang diinput harus sesuai dengan bukti transfer</span>
                                </li>
                            </ul>
                        </div>
                    </div>
    ...
    <script>
        function openBayarModal(booking) {
            ...
            if (booking.buka_jadwal) {
                ...
                const tanggal = new Date(booking.buka_jadwal.tanggal);
                document.getElementById('modal_tanggal').textContent = tanggal.toLocaleDateString('id-ID', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                });
            } else {
                document.getElementById('modal_jenis').textContent = 'Booking #' + booking.id;
                document.getElementById('modal_sesi').textContent = '-';
                document.getElementById('modal_tanggal').textContent = new Date(booking.tgl_booking).toLocaleDateString(
                    'id-ID');
            }
