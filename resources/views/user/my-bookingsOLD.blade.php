...
@section('content')
        ...
        <div id="bookingsContainer" class="space-y-4">
            @forelse($bookings as $booking)
                ...
                <div class="booking-card bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all"
                    data-search="{{ $searchKeywords }}" data-status="{{ $statusDisplay }}">
                    <div class="p-6">
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                            ...
                            <div class="flex-1">
                                <div class="flex items-start">
                                    <div
                                        class="h-14 w-14 bg-gradient-to-br from-purple-400 to-purple-600 rounded-lg flex items-center justify-center text-white mr-4 flex-shrink-0">
                                        ...
                                        @if ($booking->bukaJadwal)
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-600">
                                                ...
                                                <div class="flex items-center col-span-2">
                                                    ...
                                                    <span>Dibuat:
                                                        {{ \Carbon\Carbon::parse($booking->tgl_booking)->format('d M Y, H:i') }}</span>
                                                </div>
                                            </div>
                                        @else
                                        ...
                                        @if ($booking->keterangan)
                                            <div class="mt-3 bg-gray-50 rounded-lg p-3">
                                                ...
                                                <p class="text-sm text-gray-700">
                                                    {{ Str::limit($booking->keterangan, 100) }}</p>
                                            </div>
                                        @endif
                                        ...
                                        @if ($statusDisplay === 'pending' && $booking->tgl_expired_booking)
                                            ...
                                            @if ($daysLeft >= 0 && $daysLeft <= 3)
                                                <div class="mt-3 flex items-center gap-2 text-sm text-orange-600">
                                                    ...
                                                    <span>Segera bayar DP! Expired dalam {{ $daysLeft }} hari</span>
                                                </div>
                                            @endif
                                        @endif
                                    </div>
            ...
            <div id="noResults" class="hidden bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                ...
                <button onclick="resetFilter()"
                    class="inline-block px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors font-medium">
                    <i class="fas fa-redo mr-2"></i>Reset Filter
                </button>
            </div>
        </div>
    ...
    <div id="detailModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div
                class="border-b border-gray-200 px-6 py-4 flex items-center justify-between bg-gradient-to-r from-purple-500 to-purple-700 text-white">
                    ...
                    <div class="bg-green-50 border-l-4 border-green-500 rounded-lg p-4">
                        <div class="flex items-start">
                            ...
                            <div class="flex-1">
                                ...
                                <a href="{{ route('user.bayar') }}"
                                    class="inline-block mt-3 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm font-medium">
    ...
    <script>
        function viewDetail(data) {
            ...
            document.getElementById('detail_tanggal_booking').textContent = new Date(data.tgl_booking).toLocaleDateString(
                'id-ID', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
            ...
            document.getElementById('detail_expired').textContent = data.tgl_expired_booking ?
                new Date(data.tgl_expired_booking).toLocaleDateString('id-ID', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                }) :
            ...
            // Determine status
            const sudahDP = data.transaksi_pembayaran && data.transaksi_pembayaran.some(p => p.jenis_bayar === 'DP');
            const sudahPelunasan = data.transaksi_pembayaran && data.transaksi_pembayaran.some(p => p.jenis_bayar ===
                'Pelunasan');
            ...
            if (data.transaksi_pembayaran && data.transaksi_pembayaran.length > 0) {
                let totalBayar = 0;
                let html = '';
            ...
            if (statusDisplay === 'active' || statusDisplay === 'pending') {
                if (!sudahDP) {
                    nextAction.innerHTML =
                        '<strong>Bayar DP (Down Payment)</strong> untuk mengaktifkan booking Anda dan menghilangkan tanggal expired.';
                    nextPayment.classList.remove('hidden');
                } else if (!sudahPelunasan) {
                    const terminCount = data.transaksi_pembayaran.filter(p => p.jenis_bayar.includes('Termin')).length;
                    nextAction.innerHTML =
                        `Anda sudah membayar DP. Selanjutnya bisa bayar <strong>Termin ${terminCount + 1}</strong> atau langsung <strong>Pelunasan</strong>.`;
                    nextPayment.classList.remove('hidden');
                } else {
