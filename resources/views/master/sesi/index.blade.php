            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $item->jam_mulai }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $item->jam_selesai }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $item->keterangan ?? '-' }}</td>
                        <td class="px-6 py-4 text-center">
                            <button onclick='openModal("edit", @json($item))' class="text-blue-600 hover:text-blue-900 mx-1" title="Edit">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form action="{{ route('sesi.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus sesi {{ $item->nama }}?')">
                                @csrf
                                @method('DELETE')
